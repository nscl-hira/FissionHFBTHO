...
for run in runs:
    ...
    for i in range(numY):
        if ySub[i] < 0:
            yData[i].append(float(run[yAxis[i]]) + 1543.372624)
        else:
            yData[i].append(float(run[yAxis[i]][ySub[i]]) + 1543.372624)
...
for i in range(numY):
    ...
    # commented out legend here: subPlot[-1].legend(["data"], loc="best")
    subPlot[-1].set_ylabel("Energy (MeV)" )

#title = title[:-2] + " vs " + xAxis
title = "Normalized Energy vs q2"
#could switch which line is commented out to change between words and variable names in title**
